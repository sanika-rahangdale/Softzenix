    ...
    <PageTransition>
      <section className="min-h-[70vh] grid place-items-center px-6 py-16">
        <motion.form initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} onSubmit={submit} className="card-soft grad-border w-full max-w-md space-y-4">
          ...
          <p className="text-texts text-sm">Sign in to your Softzenix account.</p>
          ...
          <input required type="password" placeholder="Password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 outline-none focus:border-brand" />
          <button disabled={loading} className="btn-primary w-full justify-center">
            {loading ? <Loader2 className="animate-spin" size={16} /> : "Sign in"}
          </button>
          {err && <p className="text-red-400 text-sm">{err}</p>}
          <p className="text-sm text-texts text-center">No account? <Link to="/register" className="text-brand">Create one</Link></p>
        </motion.form>
      </section>
    </PageTransition>
  );
